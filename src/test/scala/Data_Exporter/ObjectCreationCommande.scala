...
object ObjectCreationCommande {
  ...
  val jddCreationCommande = jsonFile("data/jddCreationCommande.json")
  ...
  .exec { session =>
    ...
    .exec(http("Create Commercial Order")
      ...
      .headers(headers)
     // .body(ElFileBody("data/jddCreationCommande"))
      .body(StringBody(
        """
          |{
          ...
          |""".stripMargin)).asJson
      .check(status.is(200))
      .check(jsonPath("$.orders[0].code").saveAs("idOrderCancelation"))

    ).pause(4)
    .exec { session =>
      println("idOrderCancelation :" + session("idOrderCancelation").as[String])
      session
    }
    .exec(http("Unlock CheckCancelation")
      .put("/oms-orchestrator-webapp/commands/exploit/message/cancelDelayEndOrder")
      .headers(Map(
        "Content-Type" -> "application/json",
        "Authorization" -> "Basic YWRtaW5nbDpzbW9wcjQ0Z2w=",
        "Cookie" -> "JSESSIONID=66A5B71155309348FA9F362156E1BAB0"
      ))
      .body(StringBody(
        """
          |{
          |  "orderId": "${idOrderCancelation}"
          |}
          |""".stripMargin)).asJson
      .check(status.is(200))
    ).pause(4)
