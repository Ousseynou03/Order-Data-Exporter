...
object CommandeLogistique {
  ...
  val scnCommandeLogistique = scenario("Order Data Exporter Commande Logistique")
    .exec { session =>
    ...
    .exec { session =>
      println("idOrderLogistique :" + session("idOrder").as[String])
      session
    }
    .exec { session =>
      println("idCommercialLogistique :" + session("idCommercial").as[String])
      session
    }

    .exec(http("Create Commercial Order Logistique")
      .post("/order-repo/rest/v1/commercialOrders?startProcess=false")
      .headers(headers)
      .body(StringBody(
        """
          |{
          ...
          |""".stripMargin)).asJson
    .check(status.is(200))
    .check(jsonPath("$.orders[0].code").saveAs("idOrder"))
    )
    .exec {
      session =>
        println("idOrder récupéré :" + session("idOrder").as[String])
        session
    }
    .pause(5)
